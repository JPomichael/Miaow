...
namespace Miaow.Service.SSO.WebService
{
    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        TimeSpan tokenTimeOut = new TimeSpan(0, 0, Miaow.Service.SSO.WebService.TimeOutService.TokenTimeOutInSecond);

        TimeSpan loginTimeOut = new TimeSpan(0, 0, Miaow.Service.SSO.WebService.TimeOutService.LoginTimeOutTimeInSecond);

        Timer serviceCleanUpTimer = new Timer(Miaow.Service.SSO.WebService.TimeOutService.CleanUpTimeInSecond);
        ...
        public LoginResponse ValidateUser(LoginRequest loginRequest)
        {
            ...
            try
            {
                ...
                {
                    // User is not yet logged in.
                    if (!Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList.Contains(loginRequest.User.UserGuid))
                    {
                        Miaow.Service.SSO.WebService.OnLineUserService
        ...
        public void SingOut(string userName)
        {
            lock (synchronizeObjectCall)
            {
                var currentOnLineUser = Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList
        ...
        {
            return Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList;
        }
        ...
        {
            return Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList;
        }
        ...
        private void GetTokenBase(TokenRequest tokenRequest, TokenResponse response)
        {
            ...
            lock (synchronizeObjectCall)
            {
                ...
                var userIsOnline = Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList
                ...
                if (userIsOnline != null)
                {
                    ...
                    Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList.Add(securityToken);

                    //发放token.id
                  //  response.Token.TokenId = Miaow.Service.SSO.WebService.TokenBuilderService.BuilderTokenId();
                }
            }
        }
        ...
        private void ValidateTokenBase(TokenRequest tokenRequest, TokenResponse tokenResponse)
        {
            ...
            lock (synchronizeObjectCall)
            {
                ...
                if (Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList.Contains(tokenRequest.Token.TokenId))
                {
                    //存在这个token.id
                    SecurityToken securityToken = Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList[tokenRequest.Token.TokenId];
                    //一个请求token的过期时间
                    //如果大于了过期时间的话，也不会有用的
                    ...
                    //token用一欠，就删除
                    Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList.Remove(securityToken);
                }
            }
        }
        ...
        private void ServiceCleanUpTimerElapsed(object sender, ElapsedEventArgs e)
        {
            ...
            //
            var logintimeOutQuery = from users in Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList
            ...
            var tokenTimeOutQuery = from tokens in Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList
                                    where (currentDateTime - tokens.CreateTime) > tokenTimeOut
                                    select tokens;
            foreach (SecurityToken token in tokenTimeOutQuery.ToList())
            {
                Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList.Remove(token);
            }
        }
        ...
        private void DeleteUser(string userName, int id)
        {
            ...
            if (currentOnlineUser != null)
            {
                ...
                // Remove the User now
                Miaow.Service.SSO.WebService.OnLineUserService.OnLineUserList.Remove(currentOnlineUser);
            }
        }
        ...
        {
            // Delete Security Tokens
            var tokenDeleteQuery = Miaow.Service.SSO.WebService.SecurityTokenService.SecurityTokenList
