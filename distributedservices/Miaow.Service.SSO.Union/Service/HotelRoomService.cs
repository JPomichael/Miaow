...
namespace Miaow.Service.Union.Service
{
    ...
    public class HotelRoomService : IHotelRoomService
    {
        ...
        /// <returns></returns>
        public Miaow.Application.Union.Dto.HotelRoomDto GetHotelRoomById(string id)
        {
            Miaow.Application.Union.Dto.HotelRoomDto data = null;
            ...
            try
            {
                var dataStr = req.WebRequest(Miaow.Infrastructure.Crosscutting.Function.HttpMethod.GET, url, "");
                if (dataStr != null && dataStr != "")
                {
                    dataStr = dataStr.Replace("formartdata({Hotels:{Hotel:[", "");
                    dataStr = dataStr.Replace("]}})", "");
                    dataStr = dataStr.Replace("]},ElongId:\"0\"})", "");
                    data = Newtonsoft.Json.JsonConvert.DeserializeObject<Miaow.Application.Union.Dto.HotelRoomDto>(dataStr);
                }
            }
            catch (Exception ex)
            {
