...
namespace Miaow.Infrastructure.Crosscutting.Function
{
    /// <summary>
    /// 图片处理工具集
    ...
    public class PicHelper
    {
        ...
        //            }
        //            //上传时间月份的文件夹(如:2007-1)
        //            reFilePath1 = "http://img.Miaow.cn" + sFilePath + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/";
        ...
        //                    //生成缩略图550
        //                    tFilePath = sFilePath + "Miaow_" + sFileName;
        ...
        //                {
        //                    string wFilePath = sFilePath + "Miaow" + sFileName;//水印图
        ...
        public static string ClientUploadPic(HttpPostedFile oFile, int IsWater, int IsSmall, int iType)
        {
            try
            {
                ...
                if (sFileName != "")
                {
                    ...
                    //上传时间月份的文件夹(如:2007-1)
                    reFilePath1 = "http://img1.Miaow.cn" + sFilePath + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/";
                    sFilePath = "~/Upload" + sFilePath + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/";
        ...
        public static string[] DownPhoto(string strUrl, string strPath, string strTxt)
        {
            try
            {
                if (strUrl != "")
                {
                    ...
                    string strReFile = "http://img.Miaow.cn" + ConfigurationManager.AppSettings["RemotePath"] + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/";
                    ...
                    if (strTxt.ToLowerInvariant() == ".jpg" || strTxt.ToLowerInvariant() == ".gif" || strTxt.ToLowerInvariant() == ".bmp" || strTxt.ToLowerInvariant() == ".png")
                    {
                        ...
                        //生成缩略图640
                        tFilePath = sFilePath + "Miaow" + sFileName;
                        MakeImage(strFilePath, tFilePath, 640, 0, "w");
                    }
                    string strw = System.Web.HttpContext.Current.Server.MapPath("/UpLoad/") + "water.gif";
                    if (ConfigurationManager.AppSettings["WaterMark"].ToString() == "1")
                    {
                        string sFilePath = System.Web.HttpContext.Current.Server.MapPath(strPath);
                        string strFilePath = System.IO.Path.Combine(sFilePath, "Miaow" + sFileName);
        ...
        //            //上传时间月份的文件夹(如:2007-1)
        //            //edit by yjihrp
        //            //string reFileP = "http://flv.Miaow.cn" + sFilePath + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/Source/";
        //            string reFileP = "http://sys.Miaow.cn/upload" + sFilePath + "/" + DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "/Source/";
        ...
        public static void getAllFiles(string path, string type)
        {
            ...
            foreach (FileInfo fChild in dir.GetFiles()) //设置文件类型
            {
                try
                {
                    ...
                    //生成缩略图550
                    tFilePath = CutPath + "Miaow" + sFileName;
